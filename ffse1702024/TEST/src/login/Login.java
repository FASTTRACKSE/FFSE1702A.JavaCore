...
public class lophoc extends JFrame {
	...
	void getDataToTextField() {
		...
		Lbma.setText((String) jtable.getValueAt(row, 1));
		nhapdiem.setText((String) jtable.getValueAt(row, 5));
	}

	public ResultSet some() {
		String cc = (String) lop.getSelectedItem();
		ResultSet result2 = null;
		String sql = "SELECT id,maSV,tenSV,lop,email FROM sinhvien Where lop = '" + cc + "'";
		try {
	...
	ActionListener viewds = new ActionListener() {
		public void actionPerformed(ActionEvent arg0) {
			...
			Lbki.setEnabled(false);
			nhapmon.setEnabled(false);
			Lbmon.setEnabled(false);
			Lbma.setEnabled(false);
